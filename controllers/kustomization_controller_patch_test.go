...
metadata:
  ...
  labels:
	...
	Context("Kustomize functions", func() {
		var (
			namespace        *corev1.Namespace
			kubeconfig       *kustomizev1.KubeConfig
			artifactFile     string
		...
		BeforeEach(func() {
			...
			artifactFile = "patch-" + randStringRunes(5)
			artifactChecksum, err = initArtifact(artifactServer, "testdata/replacement", artifactFile)
			Expect(err).ToNot(HaveOccurred())
			artifactURL, err = artifactServer.URLForFile(artifactFile)
			Expect(err).ToNot(HaveOccurred())
		...
		It("kustomize replacement transformer", func() {
			Expect(k8sClient.Create(context.TODO(), kustomization)).To(Succeed())
