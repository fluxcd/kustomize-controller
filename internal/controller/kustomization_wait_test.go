...
`, name, data),
	...
	t.Run("emits unhealthy event", func(t *testing.T) {
		events, _ := testenv.GetEvents(ctx, k8sClient, resultK.GetName(), "", map[string]string{"kustomize.toolkit.fluxcd.io/revision": revision})
...
spec:
  ...
  template:
	...
	g.Eventually(func() bool {
		events, _ := testenv.GetEvents(ctx, k8sClient, resultK.GetName(), "", nil)
